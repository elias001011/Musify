...
class _TimeMachinePageState extends State<TimeMachinePage> {
        ...
        return ValueListenableBuilder<bool>(
          ...
          builder: (context, isEnabled, _) {
            return ValueListenableBuilder<int>(
              valueListenable: listeningStatsService.version,
  ...
  Widget _buildTimeMachine(BuildContext context) {
    ...
    final currentMonthKeys = monthKeys
        .where((monthKey) => monthKey == currentMonthKey)
        .toList();
    final historyMonthKeys = monthKeys
        .where((monthKey) => monthKey != currentMonthKey)
        .toList();
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            if (showAnnualBeforeMonths) _buildAnnualSection(context),
            for (final monthKey in currentMonthKeys)
              _buildMonthSection(context, monthKey),
            if (showAnnualRecap && !showAnnualBeforeMonths)
              _buildAnnualSection(context),
            for (final monthKey in historyMonthKeys)
              _buildMonthSection(context, monthKey),
